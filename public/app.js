...
const schemas={
contacts:{api:"contacts",title:"Clients / Ügyfelek",fields:[["name","Client name / Ügyfél neve *"],["company","Company / Cég"],["type","Type / Típus"],["email","Email"],["phone","Phone / Telefonszám"],["address","Address / Cím"],["owner","Relationship owner / Kapcsolattartó gazda"],["loss_risk","Loss risk / Elvesztési kockázat","select",["High","Medium","Low","Unknown"]],["last_contact","Last contact / Utolsó kapcsolat","date"],["next_step","Next step / Következő lépés"],["notes","Notes / Megjegyzés","textarea"]],cols:["id","name","phone","address","last_contact","next_step"]},
pianos:{api:"pianos",title:"Pianos / Zongorák",fields:[["brand","Brand / Márka"],["model","Model / Típus"],["serial_no","Serial No. / Gyári szám"],["ownership_type","Ownership / Tulajdon","select",["Customer owned","Company owned"]],["owner_contact_id","Owner Contact ID / Ügyfél ID"],["location","Location / Helyszín"],["estimated_value","Estimated value / Becsült érték - csak céges tulajdonnál","number"],["notes","Notes / Megjegyzés","textarea"]],cols:["id","brand","model","serial_no","ownership_type","owner_contact_id","location","estimated_value"]},
...
async function attachClientPianoSelector(row){
 ...
 container.innerHTML=`<label>Pianos / Zongorái</label><div id="clientPianoSelector" class="multi-box"><p class="muted">Save the client first, then assign/add pianos from the client profile if needed. / Új ügyfélnél előbb mentsd az ügyfelet, majd az ügyfélprofilban adj hozzá zongorát.</p></div>`;
 const grid=$("#form .form-grid");
 if(grid) grid.appendChild(container);
 if(!row?.id) return;
 try{
   const all=await api("/api/pianos");
   const assigned=all.filter(p=>p.owner_contact_id===row.id).map(p=>p.id);
   $("#clientPianoSelector").innerHTML=all.map(p=>`<label class="check-row"><input type="checkbox" disabled ${assigned.includes(p.id)?"checked":""}> ${p.display_name||`${p.brand||""} ${p.model||""}`} · ${p.ownership_type||p.ownership||""}</label>`).join("") + `<button type="button" class="small" onclick="clientProfile('${row.id}')">Add new piano / Új zongora hozzáadása</button>`;
 }catch(e){}
}
