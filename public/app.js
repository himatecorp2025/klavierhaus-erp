...
async function render(v){if(v==="closed_jobs"){return renderClosedJobs();}ensureView(v);if(v==="scheduler")return renderScheduler();if(v==="income_statement")return renderIncomeStatement();if(v==="finance")return renderFinance();if(v==="accounts")return renderAccounts();if(v==="users")return renderUsers();return renderTable(v)}
...
<div class="actions"><button type="button" class="ghost-btn" onclick="closeModal()">Cancel / Mégse</button><button>${row?"Save changes / Módosítás mentése":"Create job / Munka létrehozása"}</button></div>`;
 ...
 $("#form").onsubmit=async ev=>{
   ...
   if(row?.job_key){ b.job_key=row.job_key; }
...
function openJobDetails(j){
 ...
 <p class="muted"><b>Job key / Munkaazonosító:</b> ${j.job_key||j.id||""}</p>
...
function openCloseJob(j){
 $("#modalTitle").textContent="Close Job / Munka lezárása";
 $("#modal").classList.remove("hidden");
 $("#form").innerHTML=`<p class="muted">Billed amount / Számlázandó összeg kötelező. Ha 0, nem kell fájl. Ha nagyobb mint 0, fizetési mód és számla/csekk fájl kötelező.</p><div class="form-grid">
...
<div class="field"><label>Invoice number / Számla vagy csekk szám</label><input name="invoice_number"></div>
<div class="field"><label>Invoice/check file / Számla vagy csekk fájl</label><input name="file" type="file"></div>
...
<div id="nextJobFields" class="field full hidden"><h3>Next job / Következő feladat</h3><div class="form-grid"><div class="field full"><label>${req("Next title / Következő feladat neve")}</label><input name="next_title"></div><div class="field"><label>${req("Next assigned to / Következő felelős")}</label><select name="next_assigned_to"><option>Károly</option><option>Alex</option><option>Paul</option><option>Misi</option><option>Said</option></select></div><div class="field"><label>Next priority</label><select name="next_priority"><option>Critical</option><option>Urgent</option><option>High</option><option selected>Medium</option><option>Low</option></select></div><div class="field"><label>${req("Next start / Következő kezdés")}</label><input name="next_start_time" type="datetime-local"></div><div class="field"><label>${req("Next end / Következő befejezés")}</label><input name="next_end_time" type="datetime-local"></div><div class="field"><label>Next planned amount</label><input name="next_planned_amount" type="number" value="0"></div><div class="field full"><label>Next pricing basis / Következő díjmegállapítás</label><input name="next_pricing_basis"></div><div class="field full"><label>Next address / Következő cím</label><input name="next_service_address" value="${j.service_address||""}"></div><div class="field full"><label>Next instructions / Következő teendők</label><textarea name="next_instructions"></textarea></div></div></div></div><div class="actions"><button type="button" class="ghost-btn" onclick="closeModal()">Cancel</button><button>Save closeout / Lezárás mentése</button></div>`;
 $("#form").onsubmit=async e=>{
   ...
   let billed=Number(fd.get("billed_amount"));
   let file=fd.get("file");
   let payment=fd.get("payment_method");
   if(!payment){alert("Fizetési mód kötelező. / Payment method is required.");return}
   if(billed>0&&(!file||!file.name)){alert("Számla/csekk fájl kötelező, ha az összeg nagyobb mint 0.");return}
   ...
   fd.append("job_key",j.job_key||""); 
   fd.append("client_id",j.client_id||""); 
   fd.append("client_name",j.client_name||""); 
   fd.append("piano_name",j.piano_name||""); 
   fd.append("title",j.title||"");
...
async function clientProfile(id){
 ...
 $("#form").innerHTML=`<div class="work-card"><h4>${p.client.name} · ${p.client.id}</h4><p><b>Phone / Telefon:</b> ${p.client.phone||""}</p><p><b>Address / Cím:</b> ${p.client.address||""}</p><p><b>Last visit / Utolsó látogatás:</b> ${p.lastVisit||""}</p><p><b>Last job / Legutóbbi munka:</b> ${p.lastJob||""}</p><h3>Pianos / Zongorák</h3>${p.pianos.map(x=>`<p>${x.display_name||`${x.brand||""} ${x.model||""}`} · ${x.serial_no||""} · ${x.ownership_type||x.ownership||"Customer owned"}</p>`).join("")||"<p>No pianos</p>"}<h3>Jobs / Munkák</h3>${p.jobs.map(x=>`<p>${x.start_time} · ${x.title} · ${x.assigned_to} · ${x.status}</p>`).join("")||"<p>No jobs</p>"}</div><div class="actions"><button type="button" class="ghost-btn" onclick="closeModal()">Close</button></div>`;
 $("#form").onsubmit=e=>e.preventDefault()
}

function openForm(key,row=null){
 let s=schemas[key];
 $("#modal").classList.remove("hidden");
 $("#modalTitle").textContent=(row?"Edit ":"Add ")+s.title;
 $("#form").innerHTML=`<div class="form-grid">${s.fields.map(f=>field(f,row?.[f[0]])).join("")}</div><div class="actions"><button type="button" class="ghost-btn" onclick="closeModal()">Cancel</button><button>Save</button></div>`;
 if(key==="contacts"){ attachClientPianoSelector(row); }
 $("#form").onsubmit=async e=>{
   e.preventDefault();
   let body=Object.fromEntries(new FormData(e.target));
   s.fields.forEach(f=>{if(f[2]==="number")body[f[0]]=Number(body[f[0]]||0)});
   try{
     let saved;
     if(row) saved=await api(`/api/${s.api}/${row.id}`,{method:"PUT",body:JSON.stringify(body)}); 
     else saved=await api(`/api/${s.api}`,{method:"POST",body:JSON.stringify(body)});
     if(key==="contacts"){
       const clientId=(row&&row.id)||saved.id; 
       const ids=[...document.querySelectorAll('input[name="client_piano_ids"]:checked')].map(x=>x.value); 
       if(clientId) await api(`/api/contacts/${clientId}/pianos`,{method:"PUT",body:JSON.stringify({piano_ids:ids})});
     }
     closeModal();
     render(key);
   }catch(err){alert(err.message)}
...
function field(f,val=""){
 let[name,label,type,opts]=f;
 if(type==="textarea")return `<div class="field full"><label>${label}</label><textarea name="${name}">${val||""}</textarea></div>`;
 if(type==="select")return `<div class="field"><label>${label}</label><select name="${name}">${opts.map(o=>`<option ${o==val?"selected":""}>${o}</option>`).join("")}</select></div>`;
 return `<div class="field"><label>${label}</label><input name="${name}" type="${type||"text"}" value="${val??""}"></div>`
}
...
async function attachClientPianoSelector(row){
 ...
 if(!row?.id){
   $("#clientPianoSelector").innerHTML=`<p class="muted">Új ügyfélnél előbb mentsd az ügyfelet, utána szerkesztésben választható zongora.</p>`;
   return;
 }
 try{
   const all=await api("/api/pianos"); 
   const selected=all.filter(p=>p.owner_contact_id===row.id).map(p=>p.id); 
   $("#clientPianoSelector").innerHTML=`<div class="dropdown-checks">${all.map(p=>`<label class="check-row"><input type="checkbox" name="client_piano_ids" value="${p.id}" ${selected.includes(p.id)?"checked":""}> ${p.display_name||`${p.brand||""} ${p.model||""}`} · ${p.serial_no||""} · ${p.ownership_type||p.ownership||""}</label>`).join("") || "<p class='muted'>No pianos in database / Nincs zongora az adatbázisban.</p>"}</div>${renderAddForm()}`;
 }catch(e){
   $("#clientPianoSelector").innerHTML=`<p class="muted">Could not load pianos / Nem sikerült betölteni a zongorákat.</p>${renderAddForm()}`;
 }
}
async function addInlinePianoToClient(clientId){
 if(!clientId){alert("Előbb mentsd az ügyfelet. / Save the client first.");return}
 const brand=$("#newPianoBrand")?.value || "";
 const model=$("#newPianoModel")?.value || "";
 const serial_no=$("#newPianoSerial")?.value || "";
 const location=$("#newPianoLocation")?.value || "";
 const ownership_type=$("#newPianoOwnership")?.value || "Customer owned";
 const estimated_value=Number($("#newPianoValue")?.value || 0);
 ...
 try{
   ...
   await attachClientPianoSelector({id:clientId});
 }catch(err){alert(err.message)}
}
