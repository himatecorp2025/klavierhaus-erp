...
function workflowShellOpenCards(workflowId){
 const row=workshopWorkflowRows.find(w=>w.id===workflowId);
 if(isMobileAppViewport()&&row){workflowMobileDeckState={workflowId,index:0};return renderWorkshopWorkflow();}
 return WorkshopV2.open(workflowId);
}
