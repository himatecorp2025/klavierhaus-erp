...
function renderPublicArrow(direction = "external") {
  const paths = {
    external: '<path d="M5 19 19 5"/><path d="M8 5h11v11"/>',
    previous: '<path d="m14.5 5-7 7 7 7"/>',
    next: '<path d="m9.5 5 7 7-7 7"/>'
  };
  const safeDirection = Object.prototype.hasOwnProperty.call(paths, direction) ? direction : "external";
  return `<svg class="button-arrow-icon button-arrow-icon--${safeDirection}" viewBox="0 0 24 24" aria-hidden="true" focusable="false">${paths[safeDirection]}</svg>`;
}

function renderTextLink(link, language, className = "text-link") {
  if (!link || !link.label || !link.key) return "";
  if (link.key === "consultation") return `<button class="${escapeHtml(className)}" type="button" data-private-viewing-open><span>${escapeHtml(link.label)}</span><span class="button-arrow" aria-hidden="true">${renderPublicArrow("external")}</span></button>`;
  return `<a class="${className}" href="${escapeHtml(getRoute(link.key, language))}">
    <span>${escapeHtml(link.label)}</span>
    <span class="button-arrow" aria-hidden="true">${renderPublicArrow("external")}</span>
  </a>`;
}

function renderButton(link, language, variant = "primary") {
  if (!link || !link.label || !link.key) return "";
  if (link.key === "consultation") return `<button class="button button--${escapeHtml(variant)}" type="button" data-private-viewing-open><span>${escapeHtml(link.label)}</span><span class="button-arrow" aria-hidden="true">${renderPublicArrow("external")}</span></button>`;
...
function publicPianoReferenceText(item, language, { includeSerial = false } = {}) {
  const parts=[];
  if(includeSerial && item.serial_no) parts.push(`SN: ${item.serial_no}`);
  const size=String(item.size_display || [item.size_cm ? `${item.size_cm} cm` : "", item.size_in ? `(${item.size_in})` : ""].filter(Boolean).join(" ")).trim();
  if(size) parts.push(`${language === "hu" ? "Méret" : "Size"}: ${size}`);
  if(item.build_year){
    const age=Number(item.age ?? Math.max(0,new Date().getFullYear()-Number(item.build_year)));
    parts.push(language === "hu" ? `Gyártás: ${item.build_year} (${age} éves)` : `Built: ${item.build_year} (${age} ${age===1 ? "year old" : "years old"})`);
  }
  return parts.join(" · ");
}
...
function renderShowroomCollection(items, language, options = {}) {
  ...
  const cards = grouped.map(({ brand, items: brandItems }) => {
    ...
    return `<article class="catalog-card piano-brand-card" data-reveal>
      ...
      <div class="catalog-card__body"><p class="eyebrow">${escapeHtml(labels.showroomCardEyebrow || "Klavierhaus showroom")}</p><h3 class="word-safe-title"><a href="${escapeHtml(path)}">${escapeHtml(brand.label)}</a></h3>${publicPianoReferenceText(item,language) ? `<p class="piano-age">${escapeHtml(publicPianoReferenceText(item,language))}</p>` : ""}<p>${escapeHtml(`${brandItems.length} ${brandItems.length === 1 ? labels.showroomInstrumentSingular : labels.showroomInstrumentPlural}`)}</p><a class="button button--ghost" href="${escapeHtml(path)}"><span>${escapeHtml(labels.showroomDiscover || (language === "hu" ? "A hangszerek felfedezése" : "Discover the instruments"))}</span><span class="button-arrow" aria-hidden="true">${renderPublicArrow("external")}</span></a></div>
    </article>`;
  }).join("");
...
function renderDocument({ route, baseUrl, allowIndexing, nonce, homeEvents = [], reviews = [], showroomPianos = [], websiteServices = [], artists = [], landingSections = null, pageOverride = null, globalOverride = null, seoConfig = null }) {
  ...
  return `<!doctype html>
<html lang="${escapeHtml(copy.locale)}" class="no-js" data-theme="dark">
<head>
  <meta charset="utf-8">
...
function renderPublicEventCard(event, language, index = 0, labels = resolveEventCopy(language)) {
  ...
  return `<article class="public-event-card" data-reveal data-event-index="${index + 1}">
    ...
    <div class="public-event-card__body">
      ...
      <div class="public-event-card__actions"><a class="button button--ghost" href="${escapeHtml(eventPath(event, language))}">${escapeHtml(labels.details)} <span class="button-arrow" aria-hidden="true">${renderPublicArrow("external")}</span></a>${renderEventCardAction(event, language, labels)}</div>
    </div>
  </article>`;
}
...
function renderHomeEventShowcase(events, language, globalOverride = null) {
  ...
  return `<section class="section home-event-showcase" id="upcoming-events">
    ...
    <div class="event-carousel__controls" aria-label="${escapeHtml(language === "hu" ? "Események lapozása" : "Browse events")}"><button type="button" data-event-carousel-previous aria-label="${escapeHtml(language === "hu" ? "Előző események" : "Previous events")}">${renderPublicArrow("previous")}</button><button type="button" data-event-carousel-next aria-label="${escapeHtml(language === "hu" ? "Következő események" : "Next events")}">${renderPublicArrow("next")}</button></div>
    <a class="home-event-showcase__all text-link" href="${escapeHtml(getRoute("events", language))}"><span>${escapeHtml(labels.viewAll)}</span><span class="button-arrow" aria-hidden="true">${renderPublicArrow("external")}</span></a>
  </section>`;
}
...
function renderPublicEventList({ events, language, baseUrl, allowIndexing, nonce, pageOverride = null, globalOverride = null, seoConfig = null }) {
  ...
  return `<!doctype html><html lang="${escapeHtml(copy.locale)}" class="no-js" data-theme="dark"><head>${renderDynamicHead({
    language,
    title,
    description,
    canonicalUrl,
...
function renderInvitation({ invitation, token, language, baseUrl, nonce, result = "", error = "", globalOverride = null }) {
  ...
  <main id="main-content" class="invitation-shell"><section class="invitation-card" data-reveal>
    ...
    <p class="invitation-privacy">${escapeHtml(labels.privacy)}</p><a class="text-link" href="${escapeHtml(getRoute("events", language))}"><span>${escapeHtml(labels.back)}</span><span class="button-arrow" aria-hidden="true">${renderPublicArrow("external")}</span></a>
