...
function state(db, event, query = "") {
  ...
  return {
    ...
    digital,
    can_edit: digital && !closed,
    can_export: session.mode === "PAPER" || (digital && closed),
    can_close: digital && !closed,
    can_reopen: closed,
    tickets,
...
function startMode(db, event, mode, user) {
  ...
  db.transaction(() => {
    db.prepare(`UPDATE event_attendance_sessions SET mode=?,status='OPEN',started_at=COALESCE(started_at,?),started_by_user_id=COALESCE(started_by_user_id,?),last_status_change_at=?,updated_at=CURRENT_TIMESTAMP WHERE event_id=?`).run(normalized, now, user?.id || null, now, event.id);
...
function reopen(db, event, user) {
  ...
  const now = new Date().toISOString();
  db.transaction(() => {
    db.prepare(`UPDATE event_attendance_sessions SET status='OPEN',reopened_at=?,reopened_by_user_id=?,revision=revision+1,last_status_change_at=?,updated_at=CURRENT_TIMESTAMP WHERE event_id=?`).run(now, user.id, now, event.id);
...
function changeGuestStatus(db, event, ticket, nextStatus, user) {
  ...
  db.transaction(() => {
    ...
    db.prepare("UPDATE event_attendance_sessions SET last_status_change_at=?,updated_at=CURRENT_TIMESTAMP WHERE event_id=?").run(now, event.id);
...
function close(db, event, user, force = false) {
  ...
  db.transaction(() => {
    db.prepare(`UPDATE event_attendance_sessions SET status='CLOSED',closed_at=?,closed_by_user_id=?,last_status_change_at=?,snapshot_json=?,updated_at=CURRENT_TIMESTAMP WHERE event_id=?`).run(now, user.id, now, JSON.stringify(report), event.id);
