...
async function verifyRenderer(stages){
 ...
 Object.assign(ctx,{ensureView:()=>box,api:async url=>url==='/api/workshop-shell/phases'?{stages}:{workflows:[]},loadSchedulerWorkers:async()=>[],isAdmin:()=>true,isSuperadmin:()=>true,decorateWorkflowToolbar:()=>{},workflowBindDatePicker:()=>{},bindJobDateTimePickers:()=>{},workflowBindMobileSwipe:()=>{},updateNYClock:()=>{},applyLanguageToDOM:()=>{},showError:e=>errors.push(String(e)),currentNYTimeString:()=> '10:00'});
 await ctx.renderWorkshopWorkflow();assert.deepEqual(errors,[]);assert.equal((box.innerHTML.match(/class="workflow-stage-heading"/g)||[]).length,7);assert.match(box.innerHTML,/workflow-phase-one/);assert.match(box.innerHTML,/disabled title="Phase II/);assert.doesNotMatch(box.innerHTML,/workflow-drawer|workflow-details-modal-overlay/);
