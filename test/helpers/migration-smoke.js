...
try {
  ...
  // Reproduce the deployed pre-link schema exactly: workflow_stages already
  ...
  legacyDb.exec("ALTER TABLE workflow_stages DROP COLUMN calendar_job_id");
  legacyDb.close();
  ...
  const calendarJobColumn = db.prepare("PRAGMA table_info(workflow_stages)").all().some((row) => row.name === "calendar_job_id");
  if (!calendarJobColumn) throw new Error("WORKFLOW_STAGE_CALENDAR_JOB_COLUMN_MISSING");
  const calendarJobIndex = db.prepare("PRAGMA index_list(workflow_stages)").all().some((row) => row.name === "idx_workflow_stage_calendar_job");
