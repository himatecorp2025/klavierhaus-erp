...
async function verifyWorkflowUI({base,tokens,day}){
 ...
 try{
  ...
  date(form,'start_at',day(2)+'T08:00');date(form,'final_due_at',day(7)+'T17:00');const first=form.querySelector('[data-create-phase]');date(first,'due_at',day(5)+'T12:00');
  await submit(form);assert.equal(document.querySelector('[data-wf-form=create]'),null);assert.equal(document.getElementById('workflow-v2-dialog').open,false,'successful creation closes the wizard without opening details');
  let workflow=(await window.api('/api/workshop/v2/workflows')).workflows.find(w=>w.title==='DOM-created workflow');assert.ok(workflow);assert.equal(workflow.start_at,day(2)+'T08:00');assert.equal(workflow.stages[0].due_at,day(5)+'T12:00');
  await ui.open(workflow.id);
