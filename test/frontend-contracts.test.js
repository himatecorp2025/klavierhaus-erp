...
test("workflow responsibility cards use the approved status priority and phase-only drawer", () => {
  ...
  assert.match(styles, /\.workflow-toolbar-clock/);
  assert.match(appSource, /workflow-final-deadline/);
  assert.match(appSource, /workflow-board-filter-row/);
  assert.match(appSource, /workflow-overdue-toggle/);
  ...
  assert.match(appSource, /function workflowToolbarIcon\(kind\)/);
  assert.match(appSource, /function decorateWorkflowToolbar\(box\)/);
  assert.match(appSource, /decorateWorkflowToolbar\(box\)/);
  ...
  assert.match(appSource, /workflowEventLogMarkup\(stage,0\)/);
  assert.match(appSource, /function workflowShowFullEventLog\(workflowId\)/);
  assert.match(appSource, /function workflowFullEventLogPhaseMarkup\(workflow,stage\)/);
  assert.match(appSource, /workflow-full-log-phases/);
  assert.match(appSource, /Még nem történt esemény/);
  assert.match(appSource, /is-not-required/);
  assert.match(appSource, /function workflowEventActionLabel\(action\)/);
  assert.match(appSource, /WORKFLOW_STAGE_ASSIGNEE_INHERITED/);
  ...
  assert.match(styles, /body\.workflow-event-log-open/);
  assert.match(styles, /\.workflow-shell \.workflow-drawer\{display:flex;flex-direction:column;overflow:hidden\}/);
  assert.match(styles, /\.workflow-shell \.workflow-drawer-scroll\{flex:1 1 auto;min-height:0/);
  assert.match(styles, /\.workflow-shell \.workflow-event-log-block \.workflow-text-link/);
